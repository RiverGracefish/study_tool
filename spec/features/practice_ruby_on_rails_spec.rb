...
feature 'Verify links' do
  scenario 'on the study exercise page' do
    visit study_exercise_page

    expect(page).to have_link('Typing')
    expect(page).to have_link('Ruby on Rails')
    expect(page).to have_link('Link #3')
    expect(page).to have_link('Link #4')
  end
  ...
  scenario 'go to Typing link' do
    ...
    click_link('Typing')
    expect(page).to have_link('https://typing.io/lessons')
  end

  scenario 'go to Ruby on Rails link' do
    visit ruby_on_rail_path
  ...
  scenario 'go to updated class content link' do
    visit updated_class_content_path

    click_link('How do I get updated class content?')
